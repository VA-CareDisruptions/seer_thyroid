...
```{r}
a1 <- read_excel('./CONFIDENTIAL/SEERThy.23.8.21SEND.xlsx', sheet='Rate x hist x mo', skip=2) %>%
...
a2 <- read_excel('./CONFIDENTIAL/SEERThy.23.8.21SEND.xlsx', sheet='Size cat x mo PAP') %>%
  ...
  dplyr::select( Histology,date, Outcome, Population ) %>%
    mutate(hist_grp = if_else(Histology %in% c("Papillary <2cm" ), 'Pap_u_2cm',
                              if_else(Histology %in% c("Papillary 2-4cm","Papillary >4cm" ) ,'Pap_over_2cm','other'  ))) %>%
...
size1 <- read_excel('./CONFIDENTIAL/SEERThy.23.8.21SEND.xlsx',sheet='size_yr') %>%
  filter(Size <989) %>%
...
(
  ...
  (fig3c + coord_cartesian(ylim = c(-10, 300))+ ggtitle('Follicular'))  
  )/ (
  (fig3d +coord_cartesian(ylim = c(-10, 300))+ ggtitle('Oncocytic'))+
  (fig3e + coord_cartesian(ylim = c(-10, 300))+ ggtitle('Medullary'))+
  (fig3f + coord_cartesian(ylim = c(-10, 300))+ ggtitle('Anaplastic')) 
  )+
   plot_annotation(tag_levels = 'A')
